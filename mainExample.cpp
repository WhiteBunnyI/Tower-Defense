...
int main1()
{
    ...
    //Create a ball speedometer
    sf::Text speedometer;
    speedometer.setCharacterSize(20);
    speedometer.setFont(font);
    speedometer.setPosition(gameWidth/2, 0);
    ...
    while (window.isOpen())
    {
        ...
        if (isPlaying)
        {
            ...
            speedometer.setString("Ball speed: " + std::to_string(static_cast<int>(ballSpeed)));
            auto s_len = speedometer.getString().getSize() / 2;
            speedometer.setPosition(gameWidth/2 - s_len*speedometer.getCharacterSize()/2, 0);
